...
/*
    ...
    This is a misc class that is intended to ease repetitive jobs.
*/
...
public static class Misc {
	...
	static void ShowMsg(Gtk.Window? main, string title, string txt, Gtk.MessageType type)
	{
		var dlg = new Gtk.MessageDialog(
								main,
								Gtk.DialogFlags.Modal,
		...
		try {
			...
			while (!file.EndOfStream)
			{
				...
				if ( line.ToLower().StartsWith( EtqWidth, StringComparison.InvariantCulture ))
				{
					...
					if ( pos > 0 ) {
						width = Convert.ToInt32(line[ ( pos + 1) ..].Trim() );
					}
				}
				else
			...
			MsgError( window, window.Title, exc.Message );
		}
